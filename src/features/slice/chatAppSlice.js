import { createSlice } from '@reduxjs/toolkit';

export const chatAppSlice = createSlice({
  name: 'chatApp',
  initialState: {
    userName: 'Ankur',
    chatUsers: [],
    toUser: '',
    messages: [],
  },
  reducers: {
    setUserName: (state, action) => {
      state.userName = action.payload.userName;
      state.chatUsers = action.payload.chatUsers;
    },
    setChatUsers: (state, action) => {
      state.chatUsers = action.payload.chatUsers;
    },
    setToUser: (state, action) => {
      state.toUser = action.payload.toUser;
      state.messages = action.payload.messages;
    },
    setMessages: (state, action) => {
      state.messages = action.payload.messages;
    },
    insertMessage: (state, action) => {
      state.messages.push(action.payload);
    }
  },
});

export const { setUserName, setChatUsers, setToUser, setMessages, insertMessage } = chatAppSlice.actions;

// The function below is called a thunk and allows us to perform async logic. It
// can be dispatched like a regular action: `dispatch(incrementAsync(10))`. This
// will call the thunk with the `dispatch` function as the first argument. Async
// code can then be executed and other actions can be dispatched
// export const incrementAsync = amount => dispatch => {
//   setTimeout(() => {
//     dispatch(incrementByAmount(amount));
//   }, 1000);
// };

// The function below is called a selector and allows us to select a value from
// the state. Selectors can also be defined inline where they're used instead of
// in the slice file. For example: `useSelector((state) => state.counter.value)`

export const selectUserName = state => state.chatApp.userName;
export const selectToUser = state => state.chatApp.toUser;
export const selectMessages = state => state.chatApp.messages;
export const selectChatUsers = state => state.chatApp.chatUsers;

export default chatAppSlice.reducer;
